...
Assign both legacy role_id and Spatie role when creating test users:
```php
$user = User::factory()->create(['role_id' => $legacyRole->id, 'status_id' => $status->id]);
$user->assignRole('bureau_master');
```
...
### UserFactory Fields
The factory uses `username` and `primary_email` (not Laravel defaults):
```php
'username' => fake()->userName(),
'primary_email' => fake()->unique()->safeEmail(),
```
...
### Staging Mail Safety
- `STAGING_MODE=true` activates `Mail::alwaysTo()` — all outbound to one address
- `STAGING_USE_SMTP=true` enables Resend delivery (otherwise defaults to `log` driver)
- Real member emails in the DB are never contacted

### Testing Inbound Mail
```bash
# Local delivery
echo "Test" | mail -s "Test (recipients: bureau)" clubcep+bureau@test.clubcep.eu

# Wait ~1 minute or run manually:
...
| File | Tests | Coverage |
|------|-------|----------|
| `FederationVisibilityTest.php` | 4 | Active/recognized/invisible scopes, admin update |

### Running All Tests
```bash
php artisan test --compact                    # All 138 tests (321 assertions)
php artisan test --filter=FederationVisibility # Federation tests
php artisan test --filter=CriticalPath        # Critical path tests
```
