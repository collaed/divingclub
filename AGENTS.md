...
- **Full procedure + incident runbook: `docs/DEPLOY.md`. Read it before any prod deploy.**
- Prod = `prod.clubcep.eu`, `/opt/deploy/apps/divingclub-prod`, supervisor `horizon-prod`. Staging = `/opt/deploy/apps/divingclub` (`horizon`), auto-deploy suspended.
- **`php` on the box is a broken PHP 8.5 (no mbstring/bcmath/intl). ALWAYS use `php8.3` / `sudo -u clubcep php8.3 …` for every deploy, cron, and maintenance command.**
- **Never commit `bootstrap/cache/*.php`** — generated by `package:discover`, env-specific; a stale committed manifest bricks the app on boot.
- Deploy = pull → `composer install --no-dev` → `rm -f bootstrap/cache/packages.php bootstrap/cache/services.php` → `php8.3 artisan package:discover` → `php8.3 artisan migrate --force` → `php8.3 artisan optimize:clear` → **health-gate `curl /health` (accept 200/299) with `git reset --hard` rollback** → restart `horizon-prod`. `deploy/auto-deploy.sh` does exactly this.
- Do NOT `config:cache` on prod (has caused outages). `optimize:clear` only.
- Flow: edit locally → `vendor/bin/pint --dirty` → `php artisan test --compact` → CI green (lint/test/build) → merge → deploy per `docs/DEPLOY.md`
