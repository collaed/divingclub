                ...
                <div class="card dc-card">
                    <div class="table-responsive">
                        <table class="table table-sm mb-0">
                            <thead><tr><th style="width:50px"></th><th>{{ __('Name') }}</th><th>{{ __('Size') }}</th><th>{{ __('Date') }}</th><th></th></tr></thead>
                            <tbody>
                            @foreach($files as $f)
                                <tr>
                                    <td>
                                        @if($f->hasThumb())
                                            <img src="{{ route('documents.thumb', $f) }}" alt="" style="max-width:40px;max-height:40px;border-radius:3px" loading="lazy">
                                        @else
