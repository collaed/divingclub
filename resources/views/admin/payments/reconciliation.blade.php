<x-layout :title="__('Bank Reconciliation')">
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4"><div class="card dc-card text-center p-3"><h5 class="text-warning">{{ $summary['matched'] }}</h5><small class="text-muted">{{ __('Matched (pending confirm)') }}</small></div></div>
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4">
            <form method="POST" action="{{ route('admin.payments.auto-match') }}">
                @csrf
                <button class="btn btn-warning w-100 mb-2">{{ __('Auto-Match Transactions') }}</button>
            </form>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-sm table-hover">
            ...
            @foreach($transactions as $tx)
                <tr class="{{ $tx->status === 'confirmed' ? 'table-success' : ($tx->status === 'matched' ? 'table-warning' : '') }}">
                    ...
                    <td><span class="badge bg-{{ $tx->status === 'confirmed' ? 'success' : ($tx->status === 'matched' ? 'warning text-dark' : 'secondary') }}">{{ ucfirst($tx->status) }}</span></td>
                    <td>
