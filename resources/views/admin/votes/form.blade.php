<x-layout :title="__('New Vote')">
    ...
    <div class="card dc-card"><div class="card-body">
        <form method="POST" action="{{ route('admin.votes.store') }}">
            @csrf
            <div class="row g-3">
                <div class="col-md-8"><label class="form-label">{{ __('Title') }} *</label><input type="text" name="title" class="form-control @error('title') is-invalid @enderror" required>@error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                <div class="col-md-4"><label class="form-label">{{ __('Mode') }} *</label>
                    <select name="mode" class="form-select @error('mode') is-invalid @enderror" required>
                        <option value="simple">{{ __('Simple (changeable)') }}</option>
                        <option value="election">{{ __('Election (anonymous, irreversible)') }}</option>
                    </select>
                    @error('mode') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-12"><label class="form-label">{{ __('Description') }}</label><textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="2"></textarea>@error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                <div class="col-md-6"><label class="form-label">{{ __('Opens At') }}</label><input type="datetime-local" name="opens_at" class="form-control @error('opens_at') is-invalid @enderror">@error('opens_at') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                <div class="col-md-6"><label class="form-label">{{ __('Closes At') }}</label><input type="datetime-local" name="closes_at" class="form-control @error('closes_at') is-invalid @enderror">@error('closes_at') <div class="invalid-feedback">{{ $message }}</div> @enderror</div>
                <div class="col-12">
                    <div class="form-check form-check-inline"><input type="hidden" name="allow_multiple" value="0"><input type="checkbox" name="allow_multiple" value="1" class="form-check-input"><label class="form-check-label">{{ __('Allow multiple selections') }}</label></div>
                ...
                <div class="col-12">
                    ...
                    <div id="options">
                        <input type="text" name="options[]" class="form-control mb-1 @error('options.0') is-invalid @enderror" placeholder="{{ __('Option 1') }}" required>
                        @error('options.0') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        <input type="text" name="options[]" class="form-control mb-1 @error('options.1') is-invalid @enderror" placeholder="{{ __('Option 2') }}" required>
                        @error('options.1') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        <input type="text" name="options[]" class="form-control mb-1 @error('options.2') is-invalid @enderror" placeholder="{{ __('Option 3 (optional)') }}">
