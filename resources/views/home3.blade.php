...
<section class="h3-hero" id="hero">
    <div class="h3-hero-bg">
        @foreach($photos as $i => $p)
            <img src="{{ asset('storage/'.$p) }}" @if($i === 0) class="active" @endif loading="{{ $i === 0 ? 'eager' : 'lazy' }}">
        @endforeach
    </div>
...
<div class="h3-mosaic h3-reveal">
    @foreach($photos->take(6) as $i => $p)
        <a href="{{ asset('storage/'.$p) }}" onclick="openGallery('h3m',{{ $i }});return false"><img src="{{ asset('storage/'.$p) }}" loading="lazy"></a>
    @endforeach
</div>
...
<div style="display:flex;gap:0;height:180px;overflow:hidden" class="h3-reveal">
    @foreach($photos->slice(4, 4) as $i => $p)
        <a href="{{ asset('storage/'.$p) }}" onclick="openGallery('h3m',{{ $i + 4 }});return false" style="flex:1;overflow:hidden;cursor:zoom-in">
            <img src="{{ asset('storage/'.$p) }}" loading="lazy" style="width:100%;height:100%;object-fit:cover;filter:brightness(.85);transition:filter .3s" onmouseover="this.style.filter='brightness(1)'" onmouseout="this.style.filter='brightness(.85)'">
        </a>
    @endforeach
</div>
@endif
...
{{-- Lightbox --}}
<div class="h3-lightbox" id="lightbox" onclick="closeLightbox()"><img id="lightboxImg" src=""></div>
