...
| REQ-05 | Remove inline closures from routes | High | 🟡 Admin routes done (6 closures moved to controllers); 7 auth closures remain in web.php |
| REQ-06 | Extract business logic to services | Medium | ✅ Done — 24 services: FeeCalculation, BankReconciliation, Medical, Backup, Theme, License, ArticleTranslation, MailAlias, EventRegistration, TripSettlement, Homogeneity, etc. |
| REQ-07 | Create model factories | Medium | ✅ Done — 15 factories (User, Event, MemberDetail, Equipment, Article, PaymentExpected, EmailLog, ThemeSetting, TripParticipant, TripReceipt, Vote, DiveSite, Season, Newsletter, EventRegistration) |
...
## v2 Updates (April 2026)

### New Dependencies
- **spatie/laravel-permission** v6 — 6 roles, 12 granular permissions (replaces hand-rolled role system)
- **laravel/horizon** v5 — Redis queue monitoring dashboard (bureau-only)
- **intervention/image** v4 — Thumbnails, avatar auto-resize to 400×400
- **resend/resend-laravel** v1 — Email delivery via Resend API (no SMTP port 25 needed)

### Inbound Mail Alias System
- Plus-addressing: `clubcep+bureau@domain`, `clubcep+event.42@domain` — zero Postfix config
- Two modes: Maildir (local, reads files directly) or IMAP (remote mailbox)
...
- PollInboundMail job runs every minute, forwarded messages logged in event Communications

### Newsletter Enhancements
- Per-slot editable teaser text and optional custom URL
- 25 SVG marine decorations with scatter button
- "Send test to me" and "Send for Comments" buttons
- EN link in each email card (bottom-left)
- Configurable article base URL for external sites
- Load-balancing across two Resend API keys (200 emails/day free)

### Translation Quality
- Source hash tracking (xxh3) for change detection
- Word count validation (30%–300% ratio)
- Retry logic (max 3 attempts), auto-flagging for admin review
- All 14 non-English locales now complete (631 JSON keys each)

### Document Library
- Drag-and-drop upload, search across all folders
- Bulk ZIP download, inline image/PDF preview
- "My Documents" section for members

### System Monitoring
- Scheduled task heartbeat table on admin dashboard (8 jobs)
- GitHub auto-update: check + one-click apply (bureau_master only)
- Horizon dashboard linked in admin menu

### Infrastructure
- App runs as dedicated user (e.g. `clubcep`), no sudo/root needed for operation
- Redis required for Horizon queue processing
- Supervisor manages Horizon daemon
- Postfix standard plus-addressing for inbound mail (zero custom config)
- `STAGING_MODE=true` + `STAGING_USE_SMTP=true` + `MAIL_ALWAYS_TO` for safe staging
