...
- **`.kiro/steering/in-place-ajax.md`** — admin sub-settings pages, management tables, and any per-row/per-section editable field must save in place via a silent AJAX call (`$request->expectsJson()`), never a full page reload. Required for new code; bring an existing page into compliance only when you're already touching it.
...
- **Schedule heartbeats** — almost every entry in `routes/console.php` pairs its `Schedule::job()`/`Schedule::call()` with `ScheduleHeartbeat::beat($task)` (table `schedule_heartbeats`, surfaced on the admin dashboard and polled by an external Kuma monitor). A bare `->after(fn () => ScheduleHeartbeat::beat(...))` fires even if the task itself failed silently — if the underlying job can catch/swallow its own exceptions (as `WeeklyBackup::handle()` used to), have it return a bool and gate `beat()`/`fail()` on the real outcome instead of chaining `->after()` unconditionally.
